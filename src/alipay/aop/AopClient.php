...
use woodlsy\pay\alipay\aop\AopEncrypt;
...
class AopClient
{
    ...
    public function pageExecute($request, $httpmethod = "POST", $appAuthToken = null)
    {
        ...
        if (method_exists($request, "getNeedEncrypt") && $request->getNeedEncrypt()) {
            ...
            // 执行加密
            $enCryptContent           = (new AopEncrypt())->encrypt($apiParams['biz_content'], $this->encryptKey);
    ...
    private function encryptJSONSignSource($request, $responseContent)
    {
        ...
        $bizContent = (new AopEncrypt())->decrypt($parsetItem->encryptContent, $this->encryptKey);
